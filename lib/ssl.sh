...
function k_ssl () {
	...
	if [ "x$EMAIL" != "x" ] ; then
		$DOCKER_COMPOSE run --rm certbot certonly --text --noninteractive --webroot -w /var/www/html -d $FQDN -m $EMAIL --agree-tos
		...
		$DOCKER_COMPOSE run --rm certbot certonly --text --noninteractive --webroot -w /var/www/html -d $FQDN -m $EMAIL --agree-tos
		$DOCKER_COMPOSE run --rm -e RENEWD_LINAGE=/etc/letsencrypt/live/$FQDN httpd /usr/bin/ct-submit.sh
	fi
	FQDN=$FQDN
